...
const int CLICKER_DIR = 33;
const int CLICKER_PWM = 8;
const int LIMIT_SWITCH = 20;
volatile unsigned long lastClick = 0;
volatile bool buttonWait = false;
...
//The topic is in quotes "" and the callback fn follows the &
ros::Subscriber<std_msgs::Bool> subclick("arm_clicker", &clickCb);    //TODO: DETERMINE CLICKER TOPIC & CLICKER CONTROL PINS
...
void setup() {
  ...
  pinMode(CLICKER_PWM, OUTPUT);
  analogWrite(CLICKER_PWM, 250);
  attachInterrupt(digitalPinToInterrupt(LIMIT_SWITCH), BUTTON_ISR, FALLING);
